const Q = require("q")
    , FS = require("fs")
    , FILE_SCHEME = "file://"
    , FILE_SCHEME_REGEX = new RegExp("^"+FILE_SCHEME, "g")
    , HTTP_SCHEME = "http://"
    , HTTP_SCHEME_REGEX = new RegExp("^"+HTTP_SCHEME, "g")
    , FTP_SCHEME = "ftp://"
    , FTP_SCHEME_REGEX = new RegExp("^"+FTP_SCHEME, "g")
...
module.exports = {
    ...
    getMetadata: function(datasource, opts){
        ...
        if(datasource instanceof Object) {
            ...
            var scheme = getScheme(datasource);
            if(FILE_SCHEME === scheme) {
                var filename = datasource.substring(FILE_SCHEME.length);
            ...
            } else if(HTTP_SCHEME === scheme) {
                throw(new Error(HTTP_SCHEME, "not supported"))
            } else if(FTP_SCHEME === scheme) {
                throw(new Error(FTP_SCHEME, "not supported"))
            } else {
...
function getScheme(datasource) {
    var ret;
    if(FILE_SCHEME_REGEX.test(datasource)) ret = FILE_SCHEME;
    if(HTTP_SCHEME_REGEX.test(datasource)) ret = HTTP_SCHEME;
    if(FTP_SCHEME_REGEX.test(datasource)) ret = FTP_SCHEME;
    return ret;
}